...
export async function handleGetTableContents(args: any) {
    try {
        if (!args?.table_name) {
            throw new AdtError(ErrorCode.InvalidParams, 'Table name is required');
        }
        const system = args?.sap_system || 'S4H';
        const maxRows = args.max_rows || 100;
        const encodedTableName = encodeURIComponent(args.table_name);
        const url = `${await getBaseUrl(system)}/z_util/z_tablecontent/${encodedTableName}?maxRows=${maxRows}`;
